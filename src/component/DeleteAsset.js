...
const DeleteAsset = () => {
  ...
  // Fetch assetId from state
  const assetIdFromState = location.state?.assetId || '';
  const [assetId] = useState(assetIdFromState);
  ...
  useEffect(() => {
    if (assetId) {
      ...
      setMessage('No asset ID provided.');
    }
  }, [assetId, academyId, API_BASE_URL]);
  ...
  return (
    <div className="delete-asset-container">
      ...
      {!assetFound ? (
        <p>{message || 'Loading asset data...'}</p>
      ) : (
        <div>
