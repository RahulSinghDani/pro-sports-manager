    ...
    const handleSearch = async () => {
        try {
            ...
            if (response.data.length === 0) {
                alert("No due date found between this date range !");
            } else {
                setRecords(response.data);
            }
        } catch (error) {
                    ...
                    <div style={{ width: '100%' }}>
                        <div className="payment-quick-filter-main">
                            ...
                            <div className="search-by-year-month">
                                ...
                                <p className="quick-reset-btn tooltip" onClick={handleRefresh}>🔄<span className="tooltip-text">Reset</span></p>
                            </div>
                            ...
                            <tbody>
                                {records.length > 0 ? (
                                    records.map((record) => (
                                        <tr
                                            key={record.id}
                                            onClick={() => handleRowClick(role, record.academy_id, record.player_id)} // Add onClick event to the row
