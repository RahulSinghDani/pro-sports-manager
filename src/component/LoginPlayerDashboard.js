...
const LoginPlayerDashboard = () => {
    ...
    const coachId = location?.state?.coachId || "defaultValue"; // Provide a sensible default.
    ...
    return (
        <div style={{ backgroundImage: `url(${cricketBg})`, backgroundSize: 'cover', minHeight: '100vh' }}>
            {/* <Link to={`/AcademyDetails/${academy_id}/Player`}>Back</Link> */}

            <h2 className="heading" style={{ textAlign: "center", background: "black", color: "white", padding: "8px 8px", width: "100%" }}>
                Player Dashboard
                {/* <Link to={`/Login`}><button id="playerDashboard" style={{ background: "rgb(30, 30, 30)", float: "right" }}>Log Out</button></Link> */}


                {
                    role === 'coach' ? (

                        <Link to={`/LoginCoachDashboard/${role}/${academy_id}/${coachId}`}>
                            <button id="playerDashboard" style={{ background: "rgb(30, 30, 30)", float: "right" }}>Back</button>
                        </Link>
                    ) : role === 'player' ? (
                        <Link to={`/Login`}>
                            <button id="playerDashboard" style={{ background: "rgb(30, 30, 30)", float: "right" }}>Log Out</button>
                        </Link>
                    ) : null
                }
            </h2>
...
export default LoginPlayerDashboard